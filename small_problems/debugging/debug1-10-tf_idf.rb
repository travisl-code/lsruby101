# Numbers not matching expected why?
# The problem with this program becomes more obvious after running it and seeing most of the
# values returned as 0. This indicates that the `tfidf` method is somehow performing a calculation
# of n * 0. The `tf` method simply finds a count of the word, but the `idf` method actually does
# division. It's using integer values for division and the Math#log function, but that is a problem
# because integer division always uses a rounded down integer. The values need to be converted to 
# floats so that the `tfidf` method has a non-0 number to multiply.
...
# Further Exploration - what other problem is there?
# There's a problem of potential integer division by 0. A conditional
# can be used to just return 0 if there is no example of the word found.
